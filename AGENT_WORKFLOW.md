...
- **实测教训**（Lint-Filter-Controlled-Experiment / D3）：
  用 `prefer-const` 写探针放入 `entry/src/test/`，未命中。
  若只看此结果，会误判为"test 被过滤"。
- **阳性对照**：同规则、同结构放入**已知被扫描**的目录
  （`entry/src/main/ets/probe/`），**同样未命中** ⇒
  规则本身未触发，而非文件被过滤。
  换用**经验证确定触发**的规则重做后，才得到决定性对照
  （main 命中 → 7 条；test 未命中）。
- **通用原则**：任何"扫描类工具是否覆盖某范围"的实验，
  **必须先做阳性对照**（同探针放入已知覆盖范围内），
  否则"未命中"无法区分"被过滤"与"规则不触发"。
- **来源**：Lint-Filter-Controlled-Experiment（D3）。
